...
ExceptionHandler(ExceptionType which)
{
	...
	switch(type) {
        ...
        //printf("filename is %s---fileID is %d\n", filename, fileID);
       ...
       // temp[size+1]='\0';
        ...
        {
        char *temp= &(kernel->machine->mainMemory[val]);
	
        numChar = SysRead(temp, size, fileID);
        //printf("temp is %s\n", temp);
        //printf("size = %d\n",size);
        //printf("numChar is %d\n", numChar);
	    }
        ...
        for(int i = 0; i <size; i++) {
            buffer = &(kernel->machine->mainMemory[val+i]);
            input[i] = *buffer;
            //printf("input[%d] is %c---buffer[%d] is %c---\n", i, input[i], i, *buffer);
            //printf("size = %d\n",size);///
        }
        numChar = SysWrite(input, size, fileID);
        //printf("numChar is %d\n", numChar);
        kernel->machine->WriteRegister(2, (int) numChar);
        kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
        kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
